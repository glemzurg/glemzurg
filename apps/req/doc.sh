...
psql "postgresql://modeling:modeling@localhost:5432/modeling" -f "$SCRIPT_PATH/database/sql/drop_schema.sql"
[ $? -ne 0 ] && exit 1

psql "postgresql://modeling:modeling@localhost:5432/modeling" -f "$SCRIPT_PATH/database/sql/schema.sql"
[ $? -ne 0 ] && exit 1
