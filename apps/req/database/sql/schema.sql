...
COMMENT ON COLUMN data_type_atomic.data_type_id IS 'The internal ID.';
COMMENT ON COLUMN data_type_atomic.constraint_type IS 'The constraints on values for this attribute.';
COMMENT ON COLUMN data_type_atomic.reference IS 'If this is a reference, the details that define it.';
COMMENT ON COLUMN data_type_atomic.enum_ordered IS 'If this is an enumeration, enumerations could be ordered, so that should be clear.';
COMMENT ON COLUMN data_type_atomic.object_class_key IS 'If this is an object, which class it is.';
COMMENT ON COLUMN data_type_atomic.details IS 'A summary description.';
COMMENT ON COLUMN data_type_atomic.uml_comment IS 'A comment that appears in the diagrams.';
...
CREATE TABLE data_type_atomic_span (
  ...
  PRIMARY KEY (data_type_id),
  CONSTRAINT fk_span_atomic FOREIGN KEY (data_type_id) REFERENCES data_type_atomic (data_type_id) ON DELETE CASCADE
);

COMMENT ON TABLE data_type_atomic_span IS 'The definition of a span for an atomic data type.';
COMMENT ON COLUMN data_type_atomic_span.data_type_id IS 'The atomic data type this is a span for.';
...
COMMENT ON COLUMN data_type_atomic_span.higher_type IS 'Whether the lower end of the span is unconstrained, open, or closed.';
COMMENT ON COLUMN data_type_atomic_span.higher_value IS 'The value that defines the lower end of the span.';
...
CREATE TABLE data_type_atomic_enum_value (
  ...
  CONSTRAINT fk_enum_atomic FOREIGN KEY (data_type_id) REFERENCES data_type_atomic (data_type_id) ON DELETE CASCADE
);

COMMENT ON TABLE data_type_atomic_enum_value IS 'A value of an attribute that is an enum.';
COMMENT ON COLUMN data_type_atomic_enum_value.data_type_id IS 'The atomic data type this is a value for.';
COMMENT ON COLUMN data_type_atomic_enum_value.value IS 'The enum value.';
COMMENT ON COLUMN data_type_atomic_enum_value.sort_order IS 'A value for keeping presentation clear in documentation.';
...
CREATE TABLE data_type (
  ...
  uml_comment text DEFAULT NULL,
  PRIMARY KEY (data_type_id),
  CONSTRAINT fk_data_type_atomic FOREIGN KEY (data_type_id) REFERENCES data_type_atomic (data_type_id) ON DELETE CASCADE
);

COMMENT ON TABLE data_type IS 'An data type for use in a class attribute or action parameter.';
COMMENT ON COLUMN data_type.data_type_id IS 'The internal ID, the atomic table is the source.';
...
COMMENT ON COLUMN data_type.collection_min IS 'If a collection and there is a minimum number of items, the minimum.';
COMMENT ON COLUMN data_type.collection_max IS 'If a collection and there is a maximum number of items, the maximum.';
COMMENT ON COLUMN data_type.details IS 'A summary description.';
COMMENT ON COLUMN data_type.uml_comment IS 'A comment that appears in the diagrams.';
...
CREATE TABLE field (
  ...
  CONSTRAINT fk_field_record FOREIGN KEY (record_data_type_id) REFERENCES data_type (data_type_id) ON DELETE CASCADE,
  CONSTRAINT fk_field_type FOREIGN KEY (field_data_type_id) REFERENCES data_type (data_type_id) ON DELETE CASCADE
);
...
-- ALTER TABLE data_type_atomic ADD CONSTRAINT fk_atomic_class FOREIGN KEY (model_key, object_class_key) REFERENCES class (model_key, class_key) ON DELETE CASCADE;
...
--  CONSTRAINT fk_attribute_type FOREIGN KEY (data_type_id) REFERENCES data_type (data_type_id) ON DELETE CASCADE
);
...
--COMMENT ON COLUMN attribute.data_type_id IS 'The kind of attribute this is.';
...
--  CONSTRAINT fk_parameter_type FOREIGN KEY (data_type_id) REFERENCES data_type (data_type_id) ON DELETE CASCADE
);
...
--COMMENT ON COLUMN parameter.data_type_id IS 'The kind of parameter this is.';
...
CREATE TABLE flow (
  ...
  uml_comment text DEFAULT NULL,
  PRIMARY KEY (flow_id),
  CONSTRAINT fk_flow_model FOREIGN KEY (model_key) REFERENCES model (model_key) ON DELETE CASCADE
);

COMMENT ON TABLE flow IS 'A documentated flow of data/communication for the system. And integration diagram.';
COMMENT ON COLUMN flow.flow_id IS 'The internal ID.';
COMMENT ON COLUMN flow.model_key IS 'The model this flow is part of.';
COMMENT ON COLUMN flow.name IS 'The unique name of the flow.';
COMMENT ON COLUMN flow.details IS 'A summary description.';
COMMENT ON COLUMN flow.uml_comment IS 'A comment that appears in the diagrams.';
...
CREATE TABLE flow_object (
  ...
  CONSTRAINT fk_object_flow FOREIGN KEY (flow_id) REFERENCES flow (flow_id) ON DELETE CASCADE
--  CONSTRAINT fk_object_class FOREIGN KEY (model_key, class_key) REFERENCES class (model_key, class_key) ON DELETE CASCADE
);

COMMENT ON TABLE flow_object IS 'An object of a flow.';
COMMENT ON COLUMN flow_object.object_id IS 'The internal ID.';
COMMENT ON COLUMN flow_object.flow_id IS 'The flow this object is part of.';
COMMENT ON COLUMN flow_object.class_key IS 'The kind of object this is.';
COMMENT ON COLUMN flow_object.name IS 'The unique name of the flow.';
COMMENT ON COLUMN flow_object.details IS 'A summary description.';
COMMENT ON COLUMN flow_object.uml_comment IS 'A comment that appears in the diagrams.';

--------------------------------------------------------------

CREATE TYPE message_box_type AS ENUM ('event', 'attribute', 'create', 'delete', 'opt', 'alt', 'loop', 'ref');
COMMENT ON TYPE message_box_type IS 'The kind of message or box this is.

The messages:

- Event. This message is an event from a state machine.
- Attribute. This message is an attribute value.
- Create. This message is the creation of an object.
- Delete. This message is the deletion of an object.

The control boxes:

- Opt. If then branch of flow.
- Alt. A switch statement in the flow.
- Loop. A loop in the flow.
- Ref. A reference to another flow, inserted into this flow.
';

CREATE TABLE flow_message_box (
  message_box_id serial,
  flow_id bigint NOT NULL,
  parent_id bigint DEFAULT NULL,
  child_num bigint NOT NULL,
  message_box_type message_box_type NOT NULL,

  from_object_id bigint DEFAULT NULL,
  to_object_id bigint DEFAULT NULL,
  association_key text DEFAULT NULL,
  event_key text DEFAULT NULL,
  attribute_key text DEFAULT NULL,
  is_for_all boolean DEFAULT NULL,

  guard text DEFAULT NULL,
  ref_flow_id bigint DEFAULT NULL,

  details text DEFAULT NULL,
  uml_comment text DEFAULT NULL,
  PRIMARY KEY (message_box_id),
  UNIQUE (flow_id, parent_id, child_num),
  CONSTRAINT fk_message_box_flow FOREIGN KEY (flow_id) REFERENCES flow (flow_id) ON DELETE CASCADE,
  CONSTRAINT fk_message_box_from FOREIGN KEY (from_object_id) REFERENCES flow_object (object_id) ON DELETE CASCADE,
  CONSTRAINT fk_message_box_to FOREIGN KEY (to_object_id) REFERENCES flow_object (object_id) ON DELETE CASCADE,
--  CONSTRAINT fk_message_box_association FOREIGN KEY (model_key, association_key) REFERENCES association (model_key, association_key) ON DELETE CASCADE,
--  CONSTRAINT fk_message_box_event FOREIGN KEY (model_key, event_key) REFERENCES event (model_key, event_key) ON DELETE CASCADE,
--  CONSTRAINT fk_message_box_attribute FOREIGN KEY (model_key, attribute_key) REFERENCES attribute (model_key, attribute_key) ON DELETE CASCADE,
  CONSTRAINT fk_message_box_ref FOREIGN KEY (ref_flow_id) REFERENCES flow (flow_id) ON DELETE CASCADE
);

COMMENT ON TABLE flow_message_box IS 'A flow tracks message between objects.
...
COMMENT ON COLUMN flow_message_box.message_box_id IS 'The internal ID.';
COMMENT ON COLUMN flow_message_box.flow_id IS 'The flow this message is part of.';
COMMENT ON COLUMN flow_message_box.parent_id IS 'The parent (this is a tree of data). Root elements have a NULL parent.';
COMMENT ON COLUMN flow_message_box.child_num IS 'The order of this element in this level of the diagram.';
COMMENT ON COLUMN flow_message_box.message_box_type IS 'The kind of message or box this is.';

COMMENT ON COLUMN flow_message_box.from_object_id IS 'The flow object this message is from.';
COMMENT ON COLUMN flow_message_box.to_object_id IS 'The flow object this message is to.';
COMMENT ON COLUMN flow_message_box.association_key IS 'Two objects must pass a message over an association between classes.';
COMMENT ON COLUMN flow_message_box.event_key IS 'If the message is an event, which event.';
COMMENT ON COLUMN flow_message_box.attribute_key IS 'If the message is an attribute, which attribute.';
COMMENT ON COLUMN flow_message_box.is_for_all IS 'If the message sent to many children of the same object.';

COMMENT ON COLUMN flow_message_box.guard IS 'The conditional statement.';
COMMENT ON COLUMN flow_message_box.ref_flow_id IS 'The flow that is internal to this flow..';

COMMENT ON COLUMN flow_message_box.details IS 'A summary description.';
COMMENT ON COLUMN flow_message_box.uml_comment IS 'A comment that appears in the diagrams.';
