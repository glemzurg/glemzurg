...
func scanUseCaseShared(scanner Scanner, seaLevelKeyPtr, mudlevelKeyPtr *string, useCaseShared *use_case.UseCaseShared) (err error) {
	if err = scanner.Scan(
		seaLevelKeyPtr,
		mudlevelKeyPtr,
...
func LoadUseCaseShared(dbOrTx DbOrTx, modelKey, seaLevelKey, mudLevelKey string) (useCaseShared use_case.UseCaseShared, err error) {

	// Keys should be preened so they collide correctly.
	modelKey, err = identity.PreenKey(modelKey)
	if err != nil {
		return use_case.UseCaseShared{}, err
	}
	...
	err = dbQueryRow(
		...
		mudLevelKey)
	if err != nil {
		return use_case.UseCaseShared{}, errors.WithStack(err)
	}
...
func AddUseCaseShared(dbOrTx DbOrTx, modelKey, seaLevelKey, mudLevelKey string, useCaseShared use_case.UseCaseShared) (err error) {
...
func UpdateUseCaseShared(dbOrTx DbOrTx, modelKey, seaLevelKey, mudLevelKey string, useCaseShared use_case.UseCaseShared) (err error) {
...
func QueryUseCaseShareds(dbOrTx DbOrTx, modelKey string) (useCaseShareds map[string]map[string]use_case.UseCaseShared, err error) {
