...
type UseCaseSharedSuite struct {
	...
	seaUseCase  use_case.UseCase
	mudUseCase  use_case.UseCase
	mudUseCaseB use_case.UseCase
}
...
func (suite *UseCaseSharedSuite) TestLoad() {
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), use_case.UseCaseShared{
		ShareType:  "include",
...
func (suite *UseCaseSharedSuite) TestAdd() {
	...
	err := AddUseCaseShared(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.seaUseCase.Key), strings.ToUpper(suite.mudUseCase.Key), use_case.UseCaseShared{
		ShareType:  "include",
		UmlComment: "UmlComment",
	})
	assert.Nil(suite.T(), err)
...
func (suite *UseCaseSharedSuite) TestUpdate() {
	...
	err := AddUseCaseShared(suite.db, suite.model.Key, suite.seaUseCase.Key, suite.mudUseCase.Key, use_case.UseCaseShared{
		ShareType:  "include",
		UmlComment: "UmlComment",
	})
	assert.Nil(suite.T(), err)

	err = UpdateUseCaseShared(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.seaUseCase.Key), strings.ToUpper(suite.mudUseCase.Key), use_case.UseCaseShared{
		ShareType:  "extend",
...
func (suite *UseCaseSharedSuite) TestQuery() {
	...
	err = AddUseCaseShared(suite.db, suite.model.Key, suite.seaUseCase.Key, suite.mudUseCaseB.Key, use_case.UseCaseShared{
		ShareType:  "extend",
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), map[string]map[string]use_case.UseCaseShared{
		"sea_use_case_key": map[string]use_case.UseCaseShared{
			"mud_use_case_key": {
				ShareType:  "include",
				UmlComment: "UmlComment",
			},
