...
func (suite *EventSuite) TestLoad() {
	...
	assert.Equal(suite.T(), state.Event{
		Key:        "key", // Test case-insensitive.
		Name:       "Name",
		Details:    "Details",
		Parameters: []state.EventParameter{{Name: "ParamA", Source: "SourceA"}, {Name: "ParamB", Source: "SourceB"}},
	}, event)
}

func (suite *EventSuite) TestAdd() {

	err := AddEvent(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.Event{
		Key:        "KeY", // Test case-insensitive.
		Name:       "Name",
		Details:    "Details",
		Parameters: []state.EventParameter{{Name: "ParamA", Source: "SourceA"}, {Name: "ParamB", Source: "SourceB"}},
...
func (suite *EventSuite) TestAddNoParams() {
	...
	err := AddEvent(suite.db, suite.model.Key, suite.class.Key, state.Event{
		Key:        "key",
		Name:       "Name",
		Details:    "Details",
		Parameters: nil,
...
func (suite *EventSuite) TestUpdate() {
	...
	err = UpdateEvent(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.Event{
		Key:        "KeY", // Test case-insensitive.
		Name:       "NameX",
		Details:    "DetailsX",
		Parameters: []state.EventParameter{{Name: "ParamAX", Source: "SourceAX"}, {Name: "ParamBX", Source: "SourceBX"}},
	})
	assert.Nil(suite.T(), err)
...
func (suite *EventSuite) TestQuery() {
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), map[string][]state.Event{
		"class_key": []state.Event{
			{
				Key:        "key",
				Name:       "Name",
				Details:    "Details",
...
func t_AddEvent(t *testing.T, dbOrTx DbOrTx, modelKey, classKey, eventKey string) (event state.Event) {

	err := AddEvent(dbOrTx, modelKey, classKey, state.Event{
		Key:        eventKey,
		Name:       "Name",
		Details:    "Details",
		Parameters: []state.EventParameter{{Name: "ParamA", Source: "SourceA"}, {Name: "ParamB", Source: "SourceB"}},
