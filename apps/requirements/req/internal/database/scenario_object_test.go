...
func (suite *ScenarioObjectSuite) TestLoad() {
	...
	assert.Equal(suite.T(), scenario.ScenarioObject{
...
func (suite *ScenarioObjectSuite) TestAdd() {
	...
	err := AddScenarioObject(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper("scenario_key"), scenario.ScenarioObject{
		Key:          "KeY", // Test case-insensitive.
		ObjectNumber: 1,
		Name:         "Name",
		NameStyle:    "name",
...
func (suite *ScenarioObjectSuite) TestUpdate() {
	...
	err := AddScenarioObject(suite.db, suite.model.Key, "scenario_key", scenario.ScenarioObject{
		Key:          "key",
		ObjectNumber: 1,
		Name:         "Name",
		NameStyle:    "name",
	...
	err = UpdateScenarioObject(suite.db, strings.ToUpper(suite.model.Key), scenario.ScenarioObject{
		Key:          "kEy", // Test case-insensitive.
		ObjectNumber: 2,
		Name:         "NameX",
		NameStyle:    "id",
...
func (suite *ScenarioObjectSuite) TestRemove() {
	...
	err := AddScenarioObject(suite.db, suite.model.Key, suite.scenario.Key, scenario.ScenarioObject{
		Key:          "key",
		ObjectNumber: 1,
		Name:         "Name",
		NameStyle:    "name",
...
func (suite *ScenarioObjectSuite) TestQuery() {
	...
	assert.Nil(suite.T(), err)
	expected := map[string][]scenario.ScenarioObject{
		"scenario_key": {
			{
				Key:          "key",
				ObjectNumber: 1,
...
func t_AddScenarioObject(t *testing.T, dbOrTx DbOrTx, modelKey, scenarioKey, scenarioObjectKey string, objectNumber uint, classKey string) (scenarioObject scenario.ScenarioObject) {

	err := AddScenarioObject(dbOrTx, modelKey, scenarioKey, scenario.ScenarioObject{
