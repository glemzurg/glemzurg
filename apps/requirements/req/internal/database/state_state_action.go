...
func scanStateAction(scanner Scanner, stateKeyPtr *string, stateAction *state.StateAction) (err error) {
...
func LoadStateAction(dbOrTx DbOrTx, modelKey, stateActionKey string) (stateKey string, stateAction state.StateAction, err error) {

	// Keys should be preened so they collide correctly.
	modelKey, err = identity.PreenKey(modelKey)
	if err != nil {
		return "", state.StateAction{}, err
	}
	...
	err = dbQueryRow(
		...
		stateActionKey)
	if err != nil {
		return "", state.StateAction{}, errors.WithStack(err)
	}
...
func AddStateAction(dbOrTx DbOrTx, modelKey, stateKey string, stateAction state.StateAction) (err error) {
...
func UpdateStateAction(dbOrTx DbOrTx, modelKey, stateKey string, stateAction state.StateAction) (err error) {
...
func QueryStateActions(dbOrTx DbOrTx, modelKey string) (stateActions map[string][]state.StateAction, err error) {
	...
	err = dbQuery(
		...
		func(scanner Scanner) (err error) {
			var stateKey string
			var stateAction state.StateAction
			if err = scanStateAction(scanner, &stateKey, &stateAction); err != nil {
				return errors.WithStack(err)
			}
			if stateActions == nil {
				stateActions = map[string][]state.StateAction{}
			}
