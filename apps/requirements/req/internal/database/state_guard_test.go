...
func (suite *GuardSuite) TestLoad() {
	...
	assert.Equal(suite.T(), state.Guard{
...
func (suite *GuardSuite) TestAdd() {
	...
	err := AddGuard(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.Guard{
		Key:     "KeY", // Test case-insensitive.
		Name:    "Name",
		Details: "Details",
	})
...
func (suite *GuardSuite) TestUpdate() {
	...
	err := AddGuard(suite.db, suite.model.Key, suite.class.Key, state.Guard{
		Key:     "key",
		Name:    "Name",
		Details: "Details",
	})
	...
	err = UpdateGuard(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.Guard{
		Key:     "KeY", // Test case-insensitive.
		Name:    "NameX",
		Details: "DetailsX",
	})
...
func (suite *GuardSuite) TestQuery() {
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), map[string][]state.Guard{
		"class_key": []state.Guard{
			{
				Key:     "key",
				Name:    "Name",
				Details: "Details",
...
func t_AddGuard(t *testing.T, dbOrTx DbOrTx, modelKey, classKey, guardKey string) (guard state.Guard) {

	err := AddGuard(dbOrTx, modelKey, classKey, state.Guard{
		Key:     guardKey,
		Name:    "Name",
		Details: "Details",
	})
