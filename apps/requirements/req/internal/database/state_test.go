...
func (suite *StateSuite) TestLoad() {
	...
	assert.Equal(suite.T(), state.State{
...
func (suite *StateSuite) TestAdd() {
	...
	err := AddState(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.State{
		Key:        "KeY", // Test case-insensitive.
		Name:       "Name",
		Details:    "Details",
		UmlComment: "UmlComment",
...
func (suite *StateSuite) TestUpdate() {
	...
	err := AddState(suite.db, suite.model.Key, suite.class.Key, state.State{
		Key:        "key",
		Name:       "Name",
		Details:    "Details",
		UmlComment: "UmlComment",
	...
	err = UpdateState(suite.db, strings.ToUpper(suite.model.Key), strings.ToUpper(suite.class.Key), state.State{
		Key:        "KeY", // Test case-insensitive.
		Name:       "NameX",
		Details:    "DetailsX",
...
func (suite *StateSuite) TestQuery() {
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), map[string][]state.State{
		"class_key": []state.State{
			{
				Key:        "key",
				Name:       "Name",
				Details:    "Details",
...
func t_AddState(t *testing.T, dbOrTx DbOrTx, modelKey, classKey, stateKey string) (state state.State) {

	err := AddState(dbOrTx, modelKey, classKey, state.State{
		Key:        stateKey,
		Name:       "Name",
		Details:    "Details",
		UmlComment: "UmlComment",
