...
func (suite *ScenarioSuite) TestLoad() {
	...
	assert.Equal(suite.T(), scenario.Scenario{
...
func (suite *ScenarioSuite) TestAdd() {
	...
	scenarioToAdd := scenario.Scenario{
		Key:     "KeY", // Test case-insensitive.
		Name:    "Name",
		Details: "Details",
		Steps: scenario.Node{
...
func (suite *ScenarioSuite) TestUpdate() {
	...
	err := AddScenario(suite.db, suite.model.Key, suite.useCase.Key, scenario.Scenario{
		Key:     "key",
		Name:    "Name",
		Details: "Details",
		Steps:   originalSteps,
	...
	err = UpdateScenario(suite.db, strings.ToUpper(suite.model.Key), scenario.Scenario{
		Key:     "KeY", // Test case-insensitive.
		Name:    "NameX",
		Details: "DetailsX",
		Steps:   updatedSteps,
...
func (suite *ScenarioSuite) TestQueryScenarios() {
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), map[string][]scenario.Scenario{
		suite.useCase.Key: {
			{
				Key:     "scenario_key",
				Name:    "Name",
...
func t_AddScenario(t *testing.T, dbOrTx DbOrTx, modelKey, scenarioKey, useCaseKey string) (scenario scenario.Scenario) {

	err := AddScenario(dbOrTx, modelKey, useCaseKey, scenario.Scenario{
		Key:     scenarioKey,
		Name:    scenarioKey,
		Details: "",
		Steps: scenario.Node{
