...
const (
	ConstraintTypeUnconstrained = "unconstrained" // Anything.
	ConstraintTypeSpan          = "span"          // A range of allowed values.
	ConstraintTypeEnumeration   = "enumeration"   // A set of allowed values.
	ConstraintTypeReference     = "reference"     // A reference to other documentation.
	ConstraintTypeObject        = "object"        // An object of a class.
)
...
func (a Atomic) Validate() error {
	return validation.ValidateStruct(&a,
		validation.Field(&a.ConstraintType, validation.Required, validation.In(ConstraintTypeUnconstrained, ConstraintTypeSpan, ConstraintTypeReference, ConstraintTypeObject, ConstraintTypeEnumeration)),
		validation.Field(&a.Reference, validation.By(func(value interface{}) error {
			ptr, ok := value.(*string)
			if !ok {
				return errors.New("Reference must be *string")
			}
			if a.ConstraintType == ConstraintTypeReference {
				if ptr == nil || *ptr == "" {
		...
		validation.Field(&a.ObjectClassKey, validation.By(func(value interface{}) error {
			...
			if a.ConstraintType == ConstraintTypeObject {
				if ptr == nil || *ptr == "" {
		...
		validation.Field(&a.Enums, validation.Required.When(a.ConstraintType == ConstraintTypeEnumeration), validation.Empty.When(a.ConstraintType != ConstraintTypeEnumeration), validation.Each(validation.By(func(value interface{}) error { enum := value.(AtomicEnum); return (&enum).Validate() }))),
		validation.Field(&a.EnumOrdered, validation.By(func(value interface{}) error {
			ptr, ok := value.(*bool)
			if !ok {
				return errors.New("EnumOrdered must be *bool")
			}
			if a.ConstraintType == ConstraintTypeEnumeration {
				if ptr == nil {
		...
		validation.Field(&a.Span, validation.By(func(value interface{}) error {
			...
			if a.ConstraintType == ConstraintTypeSpan {
				if ptr == nil {
...
func (a Atomic) String() string {
	...
	case ConstraintTypeUnconstrained:
		return "unconstrained"

	case ConstraintTypeSpan:
		if a.Span == nil {
			return "span: <nil>"
	...
	case ConstraintTypeReference:
		return "ref from " + *a.Reference

	case ConstraintTypeObject:
		return "obj of " + *a.ObjectClassKey

	case ConstraintTypeEnumeration:
		var values []string
