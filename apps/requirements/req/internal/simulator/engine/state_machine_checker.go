...
func (smc *StateMachineChecker) Check() siminst.ViolationErrors {
	var violations siminst.ViolationErrors
	var classes []*schema.ClassSimInfo
	smc.catalog.EachInScopeClassSim(func(classInfo *schema.ClassSimInfo) {
	...
	for _, classInfo := range classes {
		...
		violations = append(violations, siminst.NewStateMachineIncompleteViolation(
			classInfo.ClassKey,
