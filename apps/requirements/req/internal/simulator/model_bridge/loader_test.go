...
func (s *LoaderTestSuite) TestLoadGlobalFunctions() {
	...
	gfuncLogic := model_logic.NewLogic(gfuncKey, model_logic.LogicTypeValue, "Max of two values.", "", parsedSpec("IF x > y THEN x ELSE y", "x", "y"), nil)
	gfunc := model_logic.NewGlobalFunction(gfuncKey, "_Max", []string{"x", "y"}, gfuncLogic)
...
func (s *LoaderTestSuite) TestLoadGlobalFunctions_NoParams() {
	...
	gfuncLogic := model_logic.NewLogic(gfuncKey, model_logic.LogicTypeValue, "Status set.", "", parsedSpec(`{"pending", "active"}`), nil)
	gfunc := model_logic.NewGlobalFunction(gfuncKey, "_StatusSet", []string{}, gfuncLogic)
...
func (s *LoaderTestSuite) TestLoadActionExpressions() {
	...
	s.Require().NoError(err)
	actionReq := model_logic.NewLogic(actionReqKey, model_logic.LogicTypeAssessment, "Precondition.", "", parsedSpec("TRUE"), nil)
...
func (s *LoaderTestSuite) TestLoadQueryExpressions() {
	...
	s.Require().NoError(err)
	queryReq := model_logic.NewLogic(queryReqKey, model_logic.LogicTypeAssessment, "Precondition.", "", parsedSpec("TRUE"), nil)
	...
	s.Require().NoError(err)
	queryGuar1 := model_logic.NewLogic(queryGuarKey1, model_logic.LogicTypeQuery, "Postcondition.", "items", parsedSpec("FALSE"), nil)
...
func (s *LoaderTestSuite) TestLoadCombined() {
	...
	s.Require().NoError(err)
	gfuncLogic := model_logic.NewLogic(gfuncKey, model_logic.LogicTypeValue, "Threshold value.", "", parsedSpec("10"), nil)
	gfunc := model_logic.NewGlobalFunction(gfuncKey, "_Threshold", nil, gfuncLogic)
