...
// Build registers an extracted IR expression in the registry.
...
func (b *DefinitionBuilder) Build(expr ExtractedExpression, reg *registry.Registry) *BuildResult {
	...
	if expr.Expression == nil {
		result.Error = fmt.Errorf("nil IR expression for %s (expression not lowered before simulation)", expr.Source)
		return result
	}
	...
	case SourceModelInvariant:
		def, err := b.buildModelInvariant(expr, reg)
		result.Definition = def
		result.Error = err

	case SourceTlaDefinition:
		def, err := b.buildTlaDefinition(expr, reg)
		result.Definition = def
		result.Error = err

	case SourceActionRequires:
		def, err := b.buildActionRequires(expr, reg)
		result.Definition = def
		result.Error = err

	case SourceActionGuarantees:
		def, kind, err := b.buildActionGuarantees(expr, reg)
		result.Definition = def
		result.GuaranteeKind = kind
		result.Error = err

	case SourceQueryRequires:
		def, err := b.buildQueryRequires(expr, reg)
		result.Definition = def
		result.Error = err

	case SourceQueryGuarantees:
		def, kind, err := b.buildQueryGuarantees(expr, reg)
		result.Definition = def
		result.GuaranteeKind = kind
		result.Error = err

	case SourceGuardCondition:
		def, err := b.buildGuardExpression(expr, reg)
		result.Definition = def
		result.Error = err
...
func (b *DefinitionBuilder) buildModelInvariant(
	...
	def, err := reg.RegisterGlobalFunction(name, expr.Expression, nil)
	if err != nil {
...
func (b *DefinitionBuilder) buildTlaDefinition(
	...
	def, err := reg.RegisterGlobalFunction(expr.Name, expr.Expression, params)
	if err != nil {
...
func (b *DefinitionBuilder) buildActionGuarantees(
	...
	kind := ClassifyGuarantee(expr.Expression)
...
// where primed_expr contains a NextState node (e.g., self.count', result')
//
// A post-condition is any other boolean expression.
func ClassifyGuarantee(expr me.Expression) GuaranteeKind {
	// Check if this is an equality comparison with a primed LHS
	if cmp, ok := expr.(*me.Compare); ok {
		if cmp.Op == me.CompareEq && containsPrimedME(cmp.Left) {
...
// containsPrimedME checks if a LHS expression contains a NextState node.
// This is used specifically for classifying guarantee LHS (shallow walk).
func containsPrimedME(expr me.Expression) bool {
	switch e := expr.(type) {
	case *me.NextState:
		return true
	case *me.FieldAccess:
		return containsPrimedME(e.Base)
	case *me.TupleIndex:
		return containsPrimedME(e.Tuple)
	default:
		return false
	}
}
