...
func (s *AssociationInvariantCheckerSuite) buildChecker() (*AssociationInvariantChecker, identity.Key, identity.Key, identity.Key) {
	...
	assoc := model_class.NewAssociation(
		...
		model_class.AssociationEnd{ClassKey: jurisdictionKey, Multiplicity: helper.Must(model_class.NewMultiplicity("any"))},
		nil, "",
	)
...
func (s *AssociationInvariantCheckerSuite) TestJurisdictionReversePartnerLinkInvariant() {
	partnerClass, partnerKey := associationInvPartnerClass()
	jurisdictionClass, jurisdictionKey := associationInvJurisdictionClass()
	subdomainKey := multiplicityMustKey("domain/d/subdomain/s")
	acKey := helper.Must(identity.NewClassKey(subdomainKey, "jurisdictional_wallet_definition"))

	hostAssocKey := helper.Must(identity.NewClassAssociationKey(subdomainKey, partnerKey, jurisdictionKey, "configures_customers_for"))
	reverseAssocKey := helper.Must(identity.NewClassAssociationKey(subdomainKey, jurisdictionKey, partnerKey, "configures_customers_for"))
	...
	reverseInvKey := helper.Must(identity.NewClassAssociationInvariantKey(reverseAssocKey, "0"))
	reverseAssoc := model_class.NewAssociation(
		reverseAssocKey,
		model_class.AssociationDetails{Name: "Configures Customers For", Details: ""},
		model_class.AssociationEnd{ClassKey: jurisdictionKey, Multiplicity: anyMult},
		model_class.AssociationEnd{ClassKey: partnerKey, Multiplicity: anyMult},
	...
	reverseAssoc.SetInvariants([]model_logic.Logic{
		model_logic.NewLogic(
			...
			"",
			parsedSpec(
				`∀ p1 ∈ self._ConfiguresCustomersFor : ∀ p2 ∈ self._ConfiguresCustomersFor : ((p1 ≠ p2) ⇒ (p1.JurisdictionalWalletDefinition ≠ p2.JurisdictionalWalletDefinition))`,
			),
			nil,
		),
	})
	...
	link := simState.CreateInstance(acKey, object.NewRecord())

	bb := state.NewBindingsBuilder(simState)
	bb.AddAssociationClassHost(
		hostAssocKey,
		"Configures Customers For",
		evaluator.AssociationHostEndpoints{
			FromClassKey: partnerKey.String(),
			ToClassKey:   jurisdictionKey.String(),
		},
		"Jurisdictional Wallet Definition",
		evaluator.AssociationHostMultiplicities{},
	)
	bb.RelationContext().CreateLink(evaluator.AssociationKey(hostAssocKey.String()), partner.Attributes, jurisdiction.Attributes)
	bb.RelationContext().AddAssociationClassRow(
		evaluator.AssociationKey(hostAssocKey.String()),
		partner.Attributes,
		jurisdiction.Attributes,
		link.Attributes,
	)
	simState.AddAssociationLink(hostAssocKey, partner.ID, jurisdiction.ID, link.ID)

	s.Empty(checker.CheckInstance(jurisdiction, bb))
}
