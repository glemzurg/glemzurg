...
- `uniqueness` (optional): Array of independent uniqueness constraints. Each entry limits how often the same attribute tuple may appear among links
  - `from_attributes` (optional): Attribute SubKeys on the from class that form that constraint's tuple
  - `to_attributes` (optional): Attribute SubKeys on the to class that form that constraint's tuple
  - At least one side must be non-empty in each entry. If only one side lists attributes, the other side is unique by instance. Duplicate tuples are rejected.
- `details` (optional): Markdown description
- `uml_comment` (optional): Comment for UML diagrams
