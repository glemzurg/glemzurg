...
The actor JSON file does not conform to the expected schema.
...
1. Run `req_check --schema actor` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Actor files in the `actors/` directory: `actors/{key}.actor.json`.
...
- Run `req_check --schema actor` for the full schema
