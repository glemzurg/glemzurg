...
The class generalization JSON file does not conform to the expected schema.
...
1. Run `req_check --schema class_generalization` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Class generalization files: `domains/{domain}/subdomains/{subdomain}/class_generalizations/{key}.cgen.json`.
...
- Run `req_check --schema class_generalization` for the full schema
