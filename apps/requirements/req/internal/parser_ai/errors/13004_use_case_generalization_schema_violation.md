...
The use case generalization JSON file does not conform to the expected schema.
...
1. Run `req_check --schema use_case_generalization` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Use case generalization files in `use_case_generalizations/` directories.
...
- Run `req_check --schema use_case_generalization` for the full schema
