...
The parameter JSON file does not conform to the expected schema.
...
1. Run `req_check --schema parameter` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Parameter definitions within action, query, or event files.
...
- Run `req_check --schema parameter` for the full schema
