...
The subdomain JSON file does not conform to the expected schema.
...
1. Run `req_check --schema subdomain` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Subdomain files: `domains/{domain}/subdomains/{subdomain}/subdomain.json`.
...
- Run `req_check --schema subdomain` for the full schema
