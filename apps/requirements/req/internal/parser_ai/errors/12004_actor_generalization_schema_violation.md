...
The actor generalization JSON file does not conform to the expected schema.
...
1. Run `req_check --schema actor_generalization` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Actor generalization files: `actor_generalizations/{key}.agen.json`.
...
- Run `req_check --schema actor_generalization` for the full schema
