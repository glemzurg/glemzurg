...
The logic JSON file does not conform to the expected schema.
...
1. Run `req_check --schema logic` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Logic files: invariants, requires, guarantees, and safety rules in various directories.
...
- Run `req_check --schema logic` for the full schema
