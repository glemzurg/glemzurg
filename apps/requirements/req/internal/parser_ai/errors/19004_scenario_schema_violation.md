# Scenario Schema Violation (E19004)

The scenario JSON file does not conform to the expected schema.
...
1. Run `req_check --schema scenario` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Scenario files: `use_cases/{use_case}/scenarios/{key}.json`.
...
- Run `req_check --schema scenario` for the full schema
