...
The association JSON file does not conform to the expected schema.
...
1. Run `req_check --schema class_association` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Association files: `{from}--{to}--{name}.assoc.json` in `class_associations/` directories.
...
- Run `req_check --schema class_association` for the full schema
