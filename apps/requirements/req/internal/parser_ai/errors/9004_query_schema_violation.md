...
The query JSON file does not conform to the expected schema.
...
1. Run `req_check --schema query` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Query files: `domains/{domain}/subdomains/{subdomain}/classes/{class}/queries/{query}.json`.
...
- Run `req_check --schema query` for the full schema
