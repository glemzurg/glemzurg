...
The class JSON file does not conform to the expected schema.
...
1. Run `req_check --schema class` to see the full JSON schema
2. Compare your file against the schema requirements
3. Fix the specific violation mentioned in the error message

## File Location

Class files: `domains/{domain}/subdomains/{subdomain}/classes/{class}/class.json`.
...
- Run `req_check --schema class` for the full schema
