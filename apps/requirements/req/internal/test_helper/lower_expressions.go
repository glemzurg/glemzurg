...
// lowerTestModelExpressions walks the model tree and re-creates all ExpressionSpecs
// using convert.NewExpressionParseFunc with full context. Parse failures leave
// Expression as nil rather than returning an error — matching parser_human's behavior.
func lowerTestModelExpressions(model *req_model.Model) error {
	...
	for dKey, domain := range model.Domains {
		for sKey, subdomain := range domain.Subdomains {
			for cKey, class := range subdomain.Classes {
				if err := lowerTestClassExpressions(&class, globalFunctions, namedSets, allActions); err != nil {
...
func lowerTestClassExpressions(class *model_class.Class, globalFunctions, namedSets, allActions map[string]identity.Key) error {
...
// If the spec has no specification text, it's a no-op. Parse failures leave
// Expression as nil (graceful degradation via NewExpressionSpec).
