...
func parseUseCase(key, filename, contents string) (useCase use_case.UseCase, err error) {
	...
	useCase, err = use_case.NewUseCase(key, parsedFile.Title, parsedFile.Markdown, level, readOnly, parsedFile.UmlComment)
	if err != nil {
		return use_case.UseCase{}, err
	}
	...
	if found {
		useCase.Actors = map[string]use_case.UseCaseActor{}
		...
		for actorKey, commentAny := range actorsMap {
			...
			useCaseActor, err := use_case.NewUseCaseActor(comment)
			if err != nil {
				return use_case.UseCase{}, err
			}
	...
	if found {
		useCase.Scenarios = []scenario.Scenario{}
		...
		for scenarioKey, scenarioData := range scenariosMap {
			...
			parsedScenario, err := scenario.NewScenario(scenarioKey, name, details)
			if err != nil {
				return use_case.UseCase{}, err
			}
			...
			if found {
				...
				for i, objAny := range objectsSlice {
					...
					parsedScenario.Objects = append(parsedScenario.Objects, scenarioObject)
				}
			}
			...
			if found {
				...
				parsedScenario.Steps = node
			}

			// Add scenario to use case.
			useCase.Scenarios = append(useCase.Scenarios, parsedScenario)
		}
	}
...
func objectFromYamlData(scenarioKey string, objectI int, objectAny any) (object scenario.ScenarioObject, err error) {
	...
	object, err = scenario.NewScenarioObject(
		key,
		objectNum,
		name,
		nameStyle,
...
func generateUseCaseContent(useCase use_case.UseCase) string {
	yaml := ""
...
func generateSteps(nodes []scenario.Node, indent string) string {
	s := ""
...
func generateNode(node scenario.Node, indent string) string {
	s := indent + "- "
	if node.Loop != "" {
		s += "loop: " + node.Loop + "\n"
		subIndent := indent + "  "
