...
func (s *LogicSuite) TestParseDoubleNot() {
	...
	inner := outer.Right.(*ast.UnaryLogic)
...
func (s *LogicSuite) TestParseAndChain() {
	...
	// Left is (TRUE /\ FALSE)
	inner := outer.Left.(*ast.BinaryLogic)
	s.Equal("∧", inner.Operator)
}
...
func (s *LogicSuite) TestParseImpliesRightAssociative() {
	...
	// Right is (FALSE => TRUE)
	inner := outer.Right.(*ast.BinaryLogic)
	s.Equal("⇒", inner.Operator)
...
func (s *LogicSuite) TestPrecedenceEquivOverImplies() {
	...
	// Left is (TRUE <=> FALSE)
	equiv := implies.Left.(*ast.BinaryLogic)
	s.Equal("≡", equiv.Operator)
}
...
func (s *LogicSuite) TestPrecedenceComplexExpression() {
	...
	or := implies.Left.(*ast.BinaryLogic)
	s.Equal("∨", or.Operator)
...
func (s *LogicSuite) TestParenthesesOverridePrecedence() {
	...
	paren := and.Right.(*ast.Parenthesized)
	or := paren.Inner.(*ast.BinaryLogic)
	s.Equal("∨", or.Operator)
}
