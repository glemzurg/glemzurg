...
func (s *TupleRecordSuite) TestTupleIndex_WithExpression() {
	...
	_, ok = idx.Index.(*ast.BinaryArithmetic)
	s.True(ok, "index should be *ast.BinaryArithmetic, got %T", idx.Index)
}
...
func (s *TupleRecordSuite) TestRecordInstance_WithExpressions() {
	...
	_, ok = record.Bindings[0].Expression.(*ast.BinaryArithmetic)
	s.True(ok, "expected *ast.BinaryArithmetic, got %T", record.Bindings[0].Expression)
}
...
func (s *TupleRecordSuite) TestRecordAltered_WithAt() {
	...
	addExpr, ok := altered.Alterations[0].Expression.(*ast.BinaryArithmetic)
	s.True(ok, "expected *ast.BinaryArithmetic, got %T", altered.Alterations[0].Expression)
