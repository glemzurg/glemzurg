...
func TestGenerateSubdomainMdListsExternalDiagramClasses(t *testing.T) {
	t.Parallel()
				...
				Subdomains: map[identity.Key]model_domain.Subdomain{
					walletSub: {
						...
						Classes: map[identity.Key]model_class.Class{
							partnerKey: model_class.NewClass(partnerKey, model_class.ClassLinks{}, model_class.ClassDetails{Name: "Partner", Details: "Wallet partner."}),
						},
					},
					opsSub: {
						Key:  opsSub,
						Name: "Operations",
						Classes: map[identity.Key]model_class.Class{
							playerKey: model_class.NewClass(playerKey, model_class.ClassLinks{}, model_class.ClassDetails{Name: "Player", Details: "Operations player."}),
						},
					},
				},
			},
	...
	got := string(writer.md[walletFile])

	classesIdx := strings.Index(got, "## Classes")
	require.Positive(t, classesIdx)
	classesSection := got[classesIdx:]

	assert.Contains(t, classesSection, "- **[Partner]")
	assert.Contains(t, classesSection, "Operations::Player")
	assert.NotContains(t, classesSection, "- **[Player]")

	partnerBullet := strings.Index(classesSection, "- **[Partner]")
	playerBullet := strings.Index(classesSection, "Operations::Player")
	require.Positive(t, partnerBullet)
	require.Positive(t, playerBullet)
	assert.Less(t, partnerBullet, playerBullet, "in-subdomain classes should precede external diagram classes")
}

func TestGenerateSubdomainMdListsCrossDomainExternalDiagramClasses(t *testing.T) {
	t.Parallel()
	...
	platformCore := helper.Must(identity.NewSubdomainKey(platformDomain, "core"))
	...
	leaderboardsFile := convertKeyToFilename("subdomain", platformLeaderboards.String(), "", ".md")
	got := string(writer.md[leaderboardsFile])

	classesIdx := strings.Index(got, "## Classes")
	require.Positive(t, classesIdx)
	classesSection := got[classesIdx:]

	assert.Contains(t, classesSection, "- **[Resolver]")
	assert.Contains(t, classesSection, "Backoffice::Administrator")
	assert.NotContains(t, classesSection, "- **[Administrator]")
}
