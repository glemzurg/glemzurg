...
_Bags!BagCardinality(self.ShipsTo) = 1
```

Use `_Bags!BagCardinality` for counts in simulator-evaluated specs. There is no FiniteSets `Cardinality` builtin in the requirements evaluator. `_Bags!SetToBag(self.ShipsTo)` also works when you need a bag-of-set for bag operators; for pure counting, `BagCardinality` on the association image is enough.
...
- `_Bags!SetToBag(self.Assoc)` — one copy of each structurally distinct endpoint record  
- `_Bags!BagCardinality(self.Assoc)` — size of that image  
...
\* Empty / count
self.Assoc = {}
_Bags!BagCardinality(self.Assoc) = n
...
4. Write invariants with **quantifiers over the image**, attribute **SubKeys**, and real **`_Bags!`** operators.  
5. The simulator coerces association navigations to endpoint sets for set/bag ops, and projects AC fields with sole/binder awareness.
